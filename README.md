...
4. Установка библиотек проекта: `pip install -r lib.txt`;
5. Изменение переменных в настройках проекта:
```python
...
```
6. Создание папку media в корневом каталоге проекта:
   1. Директория media отвечает за загрузку image файлов перевала;
7. Запуск тестов:
   1. Для начала перейти в директорию с файлом manage.py: `cd pereval`;
   2. Запустить тесты: `python manage.py test`;
   
Все, проект полностью готов к работе, запуск проекта:
1. Перейти в директорию с файлом manage.py: `cd pereval`
...
## _ВНИМАНИЕ_
___
При развертывании проекта на сервере рекомендуется удалить содержимое папки migrations в директории pereval_app.